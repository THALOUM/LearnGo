...
func main() {
	...
	for person,value := range people {
		fmt.Printf("--------------%v's hobbies--------------\n", person)
		for index, hobby := range value{
			fmt.Printf("hobby: %v\t %v\n", index+1, hobby)
		}
	}
	fmt.Println("**************************************************")

	// Exercise#50: add a record:
	people[`fleming_ian`] = []string {`steaks`, `cigars`, `espionage`}
	for person,value := range people {
		fmt.Printf("--------------%v's hobbies--------------\n", person)
		for index, hobby := range value{
			fmt.Printf("hobby: %v\t %v\n", index+1, hobby)
	...
	delete(people, `moneypenny_jenny`)
	for person,value := range people {
		fmt.Printf("--------------%v's hobbies--------------\n", person)
		for index, hobby := range value{
			fmt.Printf("hobby: %v\t %v\n", index+1, hobby)
	...
	// Exercise #52 - word frequency:
	great_gatsby_words := []string {"in", "my", "younger", "and", "more", "vulnerable", "years", "my", "father", "gave", "me", "some",
	...
	"city", "for", "three", "generations.", "the", "caraways", "are", "something", "of", "a", "clan,",
	...
	"neighborhood.", "and", "so", "with", "the", "sunshine", "and", "the", "great", "bursts", "of",
	...
	for _, word := range great_gatsby_words {
		freq[word]++
	}
