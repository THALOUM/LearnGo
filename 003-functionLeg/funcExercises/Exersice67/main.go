...
needs to add a user to the `Users` map of the `MockDatastore`. If `md` were not a pointer, `SaveUser`
would receive a copy of the `MockDatastore`, and changes would only be made to that copy, not the original
`MockDatastore`.
...
Also, note that even when methods only read the receiver's data, you might still want to use a pointer
receiver for efficiency reasons, especially if the struct is large.
...
...
When the hash table grows, we allocate a new array
